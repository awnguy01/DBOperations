            ...
            if operand[1:].isdecimal():
                if int(operand[1:]) not in range(0, attributesCount):
                    if attributesCount == 1:
                        ...
                        raise Exception(
                            f'The column offset "{operand}" should be in the range (0, {attributesCount - 1})')
            else:
                # This block of code is executed for float or string
                if hasheader:
                    if operand[1:] not in header:
                        raise Exception(
                            f'column offset "{operand}" entered is not found in input data header')
                else:
                    raise Exception(
                        f'column offset "{operand}" cannot be a string as input data has no header')
...
def myselect(args: SelectArgs):
    ...
    headers: List[str] = []
    with open(args.table_file) as tf:
        headers = tf.readline().strip().upper().split(args.splitter)
    ...
    if True:
        ...
        # colstoproject = []
        # for colref in args.projections:
        #     if colref[1:].isdecimal():
        #         if int(colref[1:]) not in range(0, totalcols):
        #             raise Exception(
        #                 "The column referenced in projection by number is invalid")
        #         colstoproject.append(int(colref[1:])-1)
        #     else:
        #         if colref[1:] not in firstline.split(splitter):
        #             raise Exception(
        #                 "The column referenced in projection by name is invalid")
        ...
        with open(args.table_file) as tf:
            ...
            for line in tf:
                row = line.strip()
                if run1_query_tree(args.condition, row, headers):
                    print(row if not len(args.projections)
                          else projection_cols(row, map(lambda p: headers.index(p.upper()), args.projections)))
                    # myoutput.write(line)
            # if len(args.projections):
            #     for line in tf:
            #         if run1_query_tree(args.condition, line, arguments):
            #             line = projection_cols(line, args.projections)
            #             print(line)
            #             # myoutput.write(line)
    ...
    #                     raise Exception(
    #                         "The column referenced in projection by number is not invalid")
    #                 colstoproject.append(colrefi - 1)
    #             else:
    #                 raise Exception(
    #                     "The column referenced in projection by name is a string so invalid")
...
def projection_cols(line, projectedcols):
    ...
    """
    rowdata = line.split(',')
    result = ",".join([rowdata[columnNumber]
                      for columnNumber in projectedcols])
    return result + '\n'
...
def main():
    try:
        ...
        select_args = SelectArgs(
            arguments[0], arguments[1], arguments[4], arguments[5])
        myselect(select_args)
